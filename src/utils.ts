...
const hasPermission = (user: Unmei.UserType, permission: string) => {
...
const hasAccessToAdminPanel = (user: Unmei.UserType) => user.is_superuser || user.group.is_superuser || hasPermission(user, 'admin_panel')

const generateClassName = (baseClassName: string, ...classNames: string[]) => {
    if(classNames.filter(name => name.length > 0).length === 0) return baseClassName;
    return `${baseClassName} ${classNames.filter(name => name.length > 0).join(' ')}`;
}
